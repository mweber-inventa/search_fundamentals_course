...
def autocomplete():
    ...
    if request.method == 'GET':  # a query has been submitted
        ...
        if prefix is not None:
            ...
            if type == "queries":
                print("autocomplete")
                search_response = opensearch.search(body=query_obj, index="bbuy_queries", explain=True)
            elif type == "products":
                print("instant search")
                search_response = opensearch.search(body=query_obj, index="bbuy_products", explain=True)
